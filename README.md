...
MDRealtime pairs a live rendered Markdown editor with local file change monitoring. When an AI agent or background script writes to a document on disk, the view updates in place, preserves your current scroll offset, and highlights modified passages. When you edit the document directly, MDRealtime applies an operating system write lock and drops a companion indicator file so agents pause their file edits until you finish.
...
- Blank document drafting. Opening an empty file displays an input canvas with helpful placeholder hints and full formatting toolbar support.
- Local revision backups. Saving creates a timestamped copy in `~/.local/share/mdrealtime/backups/` before writing new content.
- Multilingual spellchecking. Configured for Latin American Spanish, Brazilian Portuguese, and English dictionaries.

## Architecture

MDRealtime consists of two main pieces:

1. Local Python service (`bin/mdrealtime`). A lightweight HTTP daemon built on Python's standard `http.server`. It hosts frontend assets, tracks file statistics (`mtime` and byte size) with a sub-second loop, manages file permissions, and streams SSE events to connected clients.
2. Web interface (`static/` and `vendor/`). A browser client rendered inside a standalone app window using Chromium or Google Chrome with `--app` and `--class=mdrealtime`. Markdown parsing relies on `marked.js`, word-level difference calculations use `diff.js`, and HTML-to-Markdown conversion uses `turndown` with the GitHub Flavored Markdown plugin.
...
```bash
git clone https://github.com/michaelsahlmann/mdrealtime.git
cd mdrealtime
./install.sh
```

The script links `bin/mdrealtime` into `~/.local/bin/mdrealtime`, registers the application desktop file in `~/.local/share/applications/`, and installs the vector icon.
...
```bash
mdrealtime /path/to/document.md
```

If MDRealtime is already open, running the command again opens the new file in an adjacent tab within the existing window.

## Cross-platform roadmap

MDRealtime currently runs on Linux using Python and Chromium application mode. To support macOS and Windows, development will proceed in phases:
